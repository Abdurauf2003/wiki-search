...
const getResults = (query) => {
    const url = `https://ja.wikipedia.org/w/api.php?action=query&list=search&prop=info&origin=*&srlimit=25&utf8=&format=json&srsearch=${query}`
...
const addResults = (sResults) => {
    ...
    sResults.forEach((result) => {
        const url = `https://ja.wikipedia.org/?curid=${result.pageid}`;
